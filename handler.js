    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (global.db.data.chats[m.chat].mute === true && !isROwner) return
...
global.dfail = (type, m, conn) => {
    ...
    if (msg) return conn.reply(m.chat, msg, m, {
        contextInfo: {
            externalAdReply: {
                title: 'ＡＫＳＥＳ ＤＩＴＯＬＡＫ', body: wm, sourceUrl: link.web, thumbnail: fs.readFileSync('./media/denied.jpg')
            }
        }
    })
}
