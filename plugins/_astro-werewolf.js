...
let handler = async (m, { conn, command, usedPrefix, args }) => {
    ...
    if (value === "create") {
        ...
        conn.adReply(m.chat, text.trim(), 'W E R E - W O L F', '', await resize(thumb, 300, 175), false, m)
        ...
        if (ww[chat].owner !== sender)
            return m.reply(`Hanya @${ww[chat].owner.split("@")[0]} yang dapat memulai permainan`, false, { mentions: [ww[chat].owner] })
        let list1 = ""
        let list2 = ""
        let player = []
        ...
        await conn.adReply(m.chat, "*⌂ W E R E W O L F - G A M E*\n\nGame telah dimulai, para player akan memerankan perannya masing masing, silahkan cek chat pribadi untuk melihat role kalian. Berhati-hatilah para warga, mungkin malam ini adalah malah terakhir untukmu", 'W E R E - W O L F', '', await resize(thumb, 300, 175), false, m)
        await run(conn, chat, ww)
    } else if (value === "vote") {
        if (!ww[chat]) 
            return m.reply("Belum ada sesi permainan")
        ...
        if (!ww[chat]) return m.reply("Tidak ada sesi permainan")
        if (ww[chat].owner !== sender)
            return m.reply(`Hanya @${ww[chat].owner.split("@")[0]} yang dapat menghapus sesi permainan ini`)
        ...
        conn.adReply(m.chat, text, 'W E R E - W O L F', '', await resize(thumb, 300, 175), false, m)
    } else {
