...
const handler = async (m, {
    ...
    try {
        ...
        if (!args[1] || selectedFileIndex < 0 || selectedFileIndex >= res.length) {
            return conn.reply(m.chat, "⚠️ Masukkan nomor yang valid. Contoh: .terabox url 1 (1 sampai " + res.length + ")", m);
        }
        ...
        if (fileInfo.size > sizeLimit) {
            return conn.reply(m.chat, "⚠️ Ukuran file melebihi batas 100MB. Ukuran file: " + formattedSize, m);
        }
        if (fileInfo.ok) {
