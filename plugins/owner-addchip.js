let handler = async (m, { conn, command, text, args }) => {
    ...
    // Determining whether to add or remove chip based on the command
    if (command === 'addchip') {
        ...
        if (chipValue > users[user].chip) {
            ...
            // Removing the specified chip from the user's account
            users[user].chip -= chipValue
...
handler.help = ['addchip', 'remchip']
handler.tags = ['owner']
handler.command = /^(add|rem)chip$/i
handler.owner = true
handler.group = true
